//{ Driver Code Starts
    ...
    class Solution{
        public Tree cloneTree(Tree tree){
         ...
         public void cloneTreeRandomPointers(Tree node, Tree clone, HashMap<Tree, Tree> hm)
         {
             if(node == null)
             {
                 return;
             }
             clone.random = hm.get(node.random);
             cloneTreeRandomPointers(node.left, clone.left, hm);
             cloneTreeRandomPointers(node.right, clone.right, hm);
         }
         
         public Tree cloneFunction(Tree node, HashMap<Tree, Tree> hm)
         {
             if(node == null)
             {
                 return null;
             }
             Tree ans = new Tree(node.data);
             ans.left = cloneFunction(node.left, hm);
             ans.right = cloneFunction(node.right, hm);
             hm.put(node, ans);
             return ans;
         }
    }