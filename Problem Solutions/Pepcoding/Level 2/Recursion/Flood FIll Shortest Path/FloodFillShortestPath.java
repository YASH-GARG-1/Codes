...
public class FloodFillShortestPath  {
    ...
    public static void main(String args[])
    {
        ...
        if(ans != null)
        {
            System.out.print("The Shortest Path in the Flood fill is of length : "+ans.len + " and");
    ...
    public static Pair findPath(int sr, int sc, int dr, int dc, String asf, int len, int [][] visited)
    {
        ...
        if(sr == dr && sc == dc)
        {
            ...
            if(asf.length() < minLen)
            {
                minLen = asf.length();
            }
            Pair ans = new Pair(asf);
            return ans;
        }
        Pair ans = null;
        visited[sr][sc] = 1;
        for(int i = 0; i < 8; i++)
        {
            Pair pair = findPath(sr+dirA[i][0], sc+dirA[i][1], dr, dc, asf+dirN[i], len+1, visited);
            if(pair != null)
            {
                if(ans == null)
                {
